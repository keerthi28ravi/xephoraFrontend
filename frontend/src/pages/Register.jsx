...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Handshake failed. Operator registry offline.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="auth-card glass-card" style={{ position: 'relative', zIndex: 10 }}>
        <div className="auth-header">
          <div className="auth-logo-sub">
            <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
              <path d="M50 10 L85 30 L85 70 L50 90 L15 70 L15 30 Z" fill="none" stroke="#ff007b" strokeWidth="4" />
              <circle cx="50" cy="50" r="10" fill="#00f0ff" />
            </svg>
          </div>
          <h3>OPERATOR REGISTRY</h3>
          <p>LINKING NEW SYNAPSE TO XEPHORA CORE</p>
        </div>
        ...
        {error && (
          <div style={{
            ...
            REGISTRY ERR // {error.toUpperCase()}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          >
            {loading ? 'REGISTRY SYNCING...' : 'INITIALIZE PROTOCOL'} <span className="btn-glow" />
          </button>
        </form>

        <div className="auth-links-row" style={{ justifyContent: 'center', gap: '10px' }}>
          <span style={{ color: 'var(--text-secondary)' }}>ALREADY LINKED?</span>
          <Link to="/login" className="auth-link">SECURE ACCESS</Link>
        </div>
